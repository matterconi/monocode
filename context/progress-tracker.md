...
## Completed (sessione corrente — Vercel API dependency boundary)

- Nuovi log Vercel production analizzati: il fix ESM ha fatto avanzare il loader, ma `/var/task/api/[...route].js` ora fallisce con `ERR_MODULE_NOT_FOUND` su `hono`, quindi il problema residuo è dependency resolution del task Vercel root, non più sintassi ESM/CJS.
- Root `package.json` aggiornato con le runtime dependency necessarie agli handler API root (`@monocode/server`, `hono`, `@hono/zod-validator`, `@clerk/backend`, `@ai-sdk/groq`, `@monocode-ai/ai`, `@monocode/db`, `ai`, `zod`) così Vercel installa dipendenze visibili a `/var/task/api/*.js`.
- Handler `api/index.ts` e `api/[...route].ts` semplificati: rimosso l'adapter `hono/vercel` dagli entrypoint e delega diretta a `app.fetch()` riscrivendo `/api` -> `/` e `/api/*` -> `/*`; la logica route resta in `apps/server/src/app.ts`.
- `bun install --ignore-scripts` rieseguito per aggiornare `bun.lock` senza attivare il preinstall Prisma già tracciato; necessario perché Vercel usa `bun install --frozen-lockfile`.
- Verifiche: `bun build ./api/index.ts "./api/[...route].ts" --target node --outdir /var/folders/zz/9h24nvs956g9sk19vx40g2yw0000gn/T/opencode/monocode-api-runtime-check` passa; smoke locale via `bun -e` e via `node` sul bundle generato raggiunge Hono (`GET /sessions`) senza crash loader e risponde JSON controllato `500 {"error":"Authentication failed"}` con env Clerk locale non autenticante; `bunx tsc --noEmit -p apps/server/tsconfig.json` passa; `bun run check` fallisce ancora solo sugli issue preesistenti già tracciati (`apps/cli/src/scripts/test-chat.ts`, `foo.ts`, script/config DB/shared e dipendenza `pg`).

## Completed (sessione corrente — Vercel Bun workspace install)
...
## Completed (sessione corrente — Vercel db package runtime boundary)

- Nuovi log Vercel production analizzati: `/api/sessions` arriva a `apps/server/src/routes/sessions.js`, ma il bare import `@monocode/db` viene risolto dal package export del workspace installato sotto `/var/task/apps/server/node_modules/@monocode/db`, che puntava ancora a `./src/index.ts`.
- Root cause confermata: Vercel/Node non deve caricare sorgente TypeScript workspace da `node_modules` a runtime; il server import è corretto come package import, ma il package boundary di `@monocode/db` esponeva un entrypoint runtime TS invece di JS.
- Patch minima: `@monocode/db` ora esporta `types: ./src/index.ts` e `import: ./dist/index.js`; `packages/db` ha uno script `build` con `bun build ./src/index.ts --target node --outdir ./dist`; il root `postinstall` esegue Prisma generate e poi il build DB, così `bun install --frozen-lockfile` prepara il runtime export usato da Vercel.
...
## Completed (sessione corrente — Vercel AI dist during function install)

- Nuovi log Vercel production analizzati: il deploy usa davvero `main` a `e8e1672`, ma Vercel esegue un secondo `Installing dependencies...` per gli output/functions dopo il `buildCommand`; in quel secondo install il root `postinstall` buildava solo `packages/db`, quindi `packages/ai/dist/index.mjs` non era garantito nel task runtime delle funzioni.
- Patch minima: root `postinstall` ora esegue `bun run --cwd packages/ai build` prima di `packages/db generate/build`, così il runtime JS dell'AI package viene prodotto anche durante il packaging delle funzioni Vercel.
- `packages/ai/dist/` aggiunto a `.gitignore` perché è output generato da install/build, come `packages/db/dist/`.
- Root `tsconfig.json` aggiornato con `lib: ["ESNext", "DOM"]` e `types: ["bun"]` per coprire i typecheck Vercel degli handler API (`Request`, `URL`, `process`, `crypto`, `console`).
- Boundary interop Vercel rifinito: `api/index.ts`, `api/[...route].ts` e `apps/server/server.ts` usano `resolvedApp` per rendere esplicito a TypeScript che l'app Hono non è `undefined` dopo il guard runtime.
- Typecheck server allineato ai log Vercel: root e `apps/server` ora dichiarano `types: ["bun", "node"]`, e il root `devDependencies` include `@types/node` esplicitamente così `process` resta disponibile nei middleware server.
