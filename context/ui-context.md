...
- Toast viewport: non-interactive absolute stack in the top-right corner (`top: 2`, `right: 2`). Toasts use the panel background, a single left border with the variant color, optional bold title, word-wrapped message, and a maximum of three visible cards; additional toasts drop the oldest visible one.
...
- Toast cards — absolute top-right notification feedback, not focusable
