...
- Required env vars for CLI browser login are `CLERK_FRONTEND_API` and `CLERK_OAUTH_CLIENT_ID`. Required server-side Clerk env vars are `CLERK_SECRET_KEY` and `CLERK_PUBLISHABLE_KEY`. `.env.example` keeps single-underscore Clerk names and does not require `CLERK_OAUTH_CLIENT_SECRET`.
