...
- `apps/web` is the public landing page. Vercel builds the shared AI runtime package during root `postinstall` so API function packaging has `packages/ai/dist/index.mjs` available, then `buildCommand` also runs `bun run --cwd packages/ai build` before `bun run --cwd apps/web build` and serves `apps/web/dist`.
...
- `packages/ai/src/models/` — shared model registry split by responsibility: `constants/models.ts` holds static model definitions/order, `schemas.ts` holds runtime Zod schemas, `types.ts` holds derived model types, and `defaults.ts` holds coding/title defaults. Each model definition separates user-facing metadata (`label`, `providerLabel`, `description`) from runtime metadata (`runtime.provider`, `runtime.modelId`) and declares supported settings/capabilities. It does not read API keys or create provider clients; Groq credentials remain server-side env vars such as `GROQ_API_KEY`.
- `apps/server/src/ai/model-resolver.ts` — server-only resolver from shared `ModelId` to concrete AI SDK Groq model via `groq(modelId)`. It validates declared model setting overrides but does not read API keys directly.
...
- `packages/ai/package.json` — package boundary exposes TypeScript source only through the `bun` condition for local Bun development, while Node/Vercel `import` and `default` resolve to `dist/index.mjs`. Do not point Node runtime exports at `src/index.ts`; keep the root `postinstall` building this package because Vercel performs a second function dependency install after `buildCommand`.
...
Chat request and message persistence validation live in `@monocode-ai/ai`: routes import `chatRequestSchema` instead of defining request schemas inline, use `storedMessagePartsSchema` before writing UI parts into Prisma JSON, and the CLI uses `storedCodingMessagesSchema` when hydrating DB messages. Modes use `modeSchema` from `@monocode-ai/ai` as the shared runtime/type source of truth, so validated `mode` values can flow into `modes[mode]` and `getToolsForMode(mode)` without casts. `Message.mode` is a first-class DB column, not metadata, because it controls message semantics and stable UI rendering.

Model selection uses `modelSchema` from `@monocode-ai/ai` as the shared runtime/type source of truth. Static model metadata is named `ModelDefinition` / `modelDefinitions` and contains product-facing metadata, runtime provider metadata, declared settings, and explicit thinking capability metadata. `thinking.kind` distinguishes registry-declared `reasoning-effort` models from `inline-tags` and `none`; GPT OSS models declare `low`/`medium`/`high`, while Qwen declares Groq's disable-capable `none`/`default` effort options. Only `thinking.toggleable` models may show the CLI thinking toggle or send `modelSettings.reasoningEffort`. The CLI stores the selected reasoning effort separately, derives selectable `/effort` options from the selected model's registry entry, and falls back to that model's default effort when switching to a model that does not support the previously selected value. When the thinking toggle is off, models that declare a `none` option send `reasoningEffort: "none"`; models without an explicit disable option omit the setting. `chatRequestSchema` defaults missing `model` to `defaultCodingModelId`, while session title generation stays on `defaultTitleModelId` so user-facing model selection changes only the coding agent and not the title mini-call. Groq Cloud is the single runtime provider boundary today: the server requires `GROQ_API_KEY`, imports `groq` from `@ai-sdk/groq`, resolves `model.runtime.modelId` through the Groq provider when `model.runtime.provider === "groq"`, and maps supported `reasoningEffort` overrides into `providerOptions.groq` only for compatible registry-declared models. Assistant model identity is UI-rendered from the request/persisted `Message.model`, not generated by the LLM, because model-generated identity text can be copied or hallucinated after switching models in the same session.

Provider/runtime failures must not mutate durable chat history. The CLI treats `useChat`'s optimistic user message as provisional: `useSessionChat` snapshots `messages` before `sendMessage()` and restores that snapshot in `onError()`, because `clearError()` only resets AI SDK `status/error`. The server persists a completed turn atomically only from the stream `onFinish` callback: user message upsert, optional first-title update, and assistant message creation run in one transaction, so provider failures and DB write failures do not leave a user message without an assistant.
...
- `Esc` uses the same cancel chain as `Ctrl+C` for dialogs and inline menus, then stops an active chat stream via the registered chat-stream capability, then clears non-empty input; it never exits the app when no layer consumes it.
...
- `useSessionChat` registers a narrow chat-stream capability with `InteractionProvider` (`isStreaming`, `stop`) so global `Esc` can abort the active AI SDK `useChat` stream without coupling keyboard policy to chat hook internals.
...
- Modal-specific components such as `SessionsDialog`, `ThemeDialog`, and `ModelDialog` compose the generic primitives and own local orchestration state such as search query. Their content components receive state through props, fetch data if needed, filter locally, and perform product-specific actions like navigation, theme selection, or model selection.
...
- `/model` opens the model modal above the current screen without route navigation. `ModelDialog` owns the query, renders `DialogSearchInput`, filters `modelOrder`/`modelDefinitions` locally, marks the active model, and calls `AgentProvider.selectModel(modelId)` on row click or Enter while closing the modal. The selection is runtime-only and affects subsequent chat requests through `useSessionChat`.
...
- CLI browser login embeds the production public Clerk values for `CLERK_FRONTEND_API` and `CLERK_OAUTH_CLIENT_ID`, so the published npm CLI can start `/login` without local env setup. Those values are public OAuth client config, not secrets, and the same env names remain supported as local/self-host overrides. Required server-side Clerk env vars are `CLERK_SECRET_KEY` and `CLERK_PUBLISHABLE_KEY`. `.env.example` keeps single-underscore Clerk names and does not require `CLERK_OAUTH_CLIENT_SECRET`.
